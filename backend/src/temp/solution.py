...
def knight_positions(n, kr, kc):
    possible_positions = []
    # knight moves in a L shape, 2 steps in either direction andthen 1 step in either direction
    for r in [-2, 2]:
        for c in [-1, 1]:
           row_pos = kr + r
           col_pos = kc + c
    ...
    for r in [-1, 1]:
        for c in [-2, 2]:
           ...
           col_pos = kc + c
           possible_positions.append((row_pos, col_pos))
    # now these positions can be out of bounds, if so dontinclude in final list
